...
import { syncVitalSigns } from './vitalSignsSyncService';
...
export const getLocalPatients = async () => {
  try {
      ...
      SELECT 
        ...
        COALESCE(
          (
            SELECT vs.newsScore 
            FROM VitalSigns vs 
            WHERE vs.hospitalizationId = h.id AND vs.isDeleted = 0
            ORDER BY vs.createdDt DESC 
            LIMIT 1
          ),
          0
        ) as newsScore
    ...
    query += ` ORDER BY 
      CASE COALESCE(
        (SELECT vs.newsScore FROM VitalSigns vs WHERE vs.hospitalizationId = h.id AND vs.isDeleted = 0 ORDER BY vs.createdDt DESC LIMIT 1),
        0
      )
        WHEN 7 THEN 1
        WHEN 6 THEN 1
    ...
    return patients.map(p => {
      ...
      return {
        ...
        status: status,  // ВЫЧИСЛЕННЫЙ СТАТУС
        newsScore: newsScore,
